...
function Tile( element ){
	...
	var pushTile = function( x, y ){
		...
		if ( x > width/4 && x < (width/4 * 3) && y > height/4 && y < (height/4 * 3) ) {
			...
			translateString += "rotateX( 0deg ) rotateY( 0deg ) translateZ( -10px )";
		}
		...
		// is the user closer to the right/left hand side?
			...
			if ( x < width - x ) {
				...
				translateString += "rotateX( 0deg ) rotateY( -10deg ) translateZ( 0px )";
		...
		// the user is closer to the top/bottom side (also the default)
			...
			if ( y < height - y ) {
				...
				translateString += "rotateX( 10deg ) rotateY( 0deg ) translateZ( 0px )";
...
var tileElements = document.getElementsByClassName( 'container-tile' );
var i;
